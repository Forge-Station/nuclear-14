...
public abstract partial class SharedPhotoSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<PhotoCameraComponent, GetMaterialWhitelistEvent>(OnGetMaterialWhitelist);
        SubscribeLocalEvent<PhotoCameraComponent, MapInitEvent>(OnCameraMapInit);
    ...
    private void OnCameraMapInit(EntityUid uid, PhotoCameraComponent component, MapInitEvent args)
    {
        if (TryComp<MaterialStorageComponent>(uid, out var storage))
            _material.UpdateMaterialWhitelist(uid, storage);
    }

    private void OnGetMaterialWhitelist(EntityUid uid, PhotoCameraComponent component, ref GetMaterialWhitelistEvent args)
    {
        if (args.Storage != uid)
            return;

        if (!args.Whitelist.Contains(component.CardMaterial))
            args.Whitelist.Add(component.CardMaterial);
    }
