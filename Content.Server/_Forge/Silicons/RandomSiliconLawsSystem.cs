...
/// <summary>
/// Forge-Change: see <see cref="RandomSiliconLawsComponent"/>. Generates a set of scrambled laws on spawn.
/// The law-generation logic below is a copy of the ion storm event's GenerateLaw (kept in _Forge so we
/// don't touch upstream), reusing the public IonStorm* datasets and locale strings.
/// </summary>
public sealed class RandomSiliconLawsSystem : EntitySystem
{
    ...
    // ion storm datasets (all start with IonStorm)
    ...
    {
        // SetLaws needs a provider to write into.
        if (!HasComp<SiliconLawProviderComponent>(ent))
            return;

        var count = _random.Next(ent.Comp.MinLaws, ent.Comp.MaxLaws + 1);
    ...
    // (copied from Content.Server/StationEvents/Events/IonStormRule.cs)
    private string GenerateLaw()
    {
        ...
        var crewAll = _random.Prob(0.5f) ? crew2 : Loc.GetString("ion-storm-crew");
        ...
        var part = Loc.GetString("ion-storm-part", ("part", _random.Prob(0.5f)));
        ...
        return _random.Next(0, 36) switch
        {
            ...
            5  => Loc.GetString("ion-storm-law-thing-harmful", ("thing", _random.Prob(0.5f) ? concept : action)),
            ...
            9  => Loc.GetString("ion-storm-law-requires", ("who", who), ("plural", plural), ("thing", _random.Prob(0.5f) ? concept : require)),
            10 => Loc.GetString("ion-storm-law-requires-subjects", ("who", who), ("plural", plural), ("joined", joined), ("subjects", triple)),
            11 => Loc.GetString("ion-storm-law-allergic", ("who", who), ("plural", plural), ("severity", allergySeverity), ("allergy", _random.Prob(0.5f) ? concept : allergy)),
            12 => Loc.GetString("ion-storm-law-allergic-subjects", ("who", who), ("plural", plural), ("severity", allergySeverity), ("adjective", adjective), ("subjects", _random.Prob(0.5f) ? objects : crew1)),
            ...
            19 => Loc.GetString("ion-storm-law-eat", ("who", crewAll), ("adjective", adjective), ("food", _random.Prob(0.5f) ? food : triple)),
            ...
            29 => Loc.GetString("ion-storm-law-crew-only-subjects", ("adjective", adjective), ("subjects", _random.Prob(0.5f) ? objectsThreats : "PEOPLE"), ("part", part)),
