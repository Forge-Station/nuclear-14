...
public sealed class TapeRecorderSystem : SharedTapeRecorderSystem
{
    ...
    protected override void ReplayMessagesInSegment(Entity<TapeRecorderComponent> ent, TapeCassetteComponent tape, float segmentStart, float segmentEnd)
    {
        ...
        foreach (var message in tape.RecordedData)
        {
            ...
            {
                var barkOverride = EnsureComp<SpeechSynthesisComponent>(ent);
                barkOverride.VoicePrototypeId = message.Bark;
                barkOverride.PlaybackSpeed = message.BarkPitch;
            }
            ...
            var language = message.Language ?? SharedLanguageSystem.Universal;

            //Play the message
            _chat.TrySendInGameICMessage(ent, message.Message, InGameICChatType.Speak, ChatTransmitRange.Normal, nameOverride: message.Name ?? ent.Comp.DefaultName, languageOverride: _proto.Index(language));

            RemComp<SpeechSynthesisComponent>(ent);
    ...
    private void OnListen(Entity<TapeRecorderComponent> ent, ref ListenEvent args)
    {
        ...
        if (TryComp<SpeechSynthesisComponent>(args.Source, out var barksComponent))
        {
            bark = barksComponent.VoicePrototypeId;
            barkPitch = barksComponent.PlaybackSpeed;
        }
        ...
        cassette.Comp.Buffer.Add(new TapeCassetteRecordedMessage(cassette.Comp.CurrentPosition, name, verb, bark, barkPitch, tts, _language.GetLanguage(args.Source), args.Message));
    }
    ...
    private void OnPrintMessage(Entity<TapeRecorderComponent> ent, ref PrintTapeRecorderMessage args)
    {
        ...
        foreach (var message in cassette.Comp.RecordedData)
        {
            ...
            var language = message.Language ?? SharedLanguageSystem.Universal;
            var languagedMessage = _language.CanUnderstand(uid, language) ? message.Message : _language.ObfuscateSpeech(message.Message, _proto.Index(language));
